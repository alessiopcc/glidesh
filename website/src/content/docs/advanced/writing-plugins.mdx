...
A `pending` response may carry an optional `diff` describing the change in detail. It is
shown only when the run was started with `--diff`, and the field can be omitted entirely —
plugins written before it existed keep working unchanged.
