...
- Return descriptive `plan` messages in `Pending` responses — they lead the dry-run output for the task, and a run counts a task as "would change" precisely when `check` returns `pending`
